...
def generate_guide():
    ...
    try:
        ...
        depth_map = {
            'quick': 'Guide length: CONCISE - 3 to 5 pages. Cover only essential steps. Brief and actionable.',
            'standard': 'Guide length: COMPREHENSIVE - 7 to 10 pages. Cover all steps with context, tips, and common mistakes.',
            'detailed': 'Guide length: EXHAUSTIVE - 12 to 15 pages. Cover every detail, edge case, and scenario.'
        }
        depth_note = depth_map.get(depth, depth_map['standard'])
        ...
        if extra_request:
            attachment_note = (
                ...
                '\nAcknowledge the attachment in the guide text with a note like '
                '"This guide includes a [template name] - see the attached file."'
            )

        system_prompt = (
            'You are an expert at creating practical DIY guides for DoItYaDamnSelf.com.\n'
            ...
            '  "category": "money-credit, life-legal, business-organizations, paperwork-filings, or transportation-logistics",\n'
            '  "description": "2-sentence description under 160 characters",\n'
            '  "tags": ["tag1", "tag2", "tag3", "tag4", "tag5"],\n'
            '  "attachment_name": "suggested filename e.g. nda-template.docx, or empty string"\n'
            '}\n\n'
            + depth_note + '\n\n'
            'Price: $2.99 simple, $3.99 moderate, $4.99 complex.\n\n'
            'CRITICAL LINK RULE: Every website, form, agency, or tool mentioned ANYWHERE must be '
            'a clickable markdown link [text](url). Never write plain URLs.\n'
            'Correct: Visit [AnnualCreditReport.com](https://www.annualcreditreport.com)\n'
            'Wrong: Visit AnnualCreditReport.com'
            + attachment_note
        )
        ...
        message = client.messages.create(
            ...
            messages=[{'role': 'user', 'content': 'Create a DIY guide for: ' + topic}]
        )
        ...
        if extra_request and attachment_type != 'none':
            try:
                ...
                if attachment_type == 'excel':
                    att_system = (
                        'Create an Excel template. Respond with JSON only:\n'
                        '{"sheets": [{"name": "Sheet name", "headers": ["Col1", "Col2"], "rows": [["example", "data"]]}]}\n'
                        'Make it practical with real example data rows.'
                    )
                    att_msg = client.messages.create(
                        model='claude-sonnet-4-20250514',
                        max_tokens=4000,
                        system=att_system,
                        messages=[{'role': 'user', 'content': 'Excel template for: ' + parsed['title'] + '. Request: ' + extra_request}]
                    )
                    att_data = _json.loads(att_msg.content[0].text)
                    ...
                    for sheet_data in att_data.get('sheets', []):
                        ...
                        for ci, header in enumerate(sheet_data.get('headers', []), 1):
                    ...
                    att_system = (
                        'Create a professional Word document template. Respond with JSON only:\n'
                        '{"title": "Document title", "sections": [{"heading": "Section", "content": "text with [PLACEHOLDER] for fillable parts"}]}\n'
                    ...
                    att_msg = client.messages.create(
                        ...
                        max_tokens=6000,
                        system=att_system,
                        messages=[{'role': 'user', 'content': 'Word template for: ' + parsed['title'] + '. Request: ' + extra_request}]
                    )
                    att_text_raw = att_msg.content[0].text
                    try:
                        att_data = _json.loads(att_text_raw)
                    except Exception:
                        match = _re.search(r'\{[\s\S]*\}', att_text_raw)
                    ...
                    doc = Document()
                    doc.sections[0].header.paragraphs[0].text = 'DoItYaDamnSelf.com'
                    doc.sections[0].header.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
                    h = doc.add_heading(att_data.get('title', parsed['title']), 0)
                    h.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    doc.add_paragraph()
                    for section in att_data.get('sections', []):
                        doc.add_heading(section.get('heading', ''), level=1)
                        doc.add_paragraph(section.get('content', ''))
                    doc.sections[0].footer.paragraphs[0].text = '© DoItYaDamnSelf.com — For personal use only. Not legal advice.'
                ...
                if att_bytes:
                    if not attachment_name:
                        safe = parsed['title'].lower().replace(' ', '-').replace('(', '').replace(')', '').replace("'", '')
                        ext = '.xlsx' if attachment_type == 'excel' else '.docx'
                        attachment_name = safe + '-template' + ext
                    attachment_url = upload_to_dropbox(att_bytes, attachment_name)
                    print('Attachment uploaded: ' + attachment_name)

            except Exception as e:
                print('Attachment generation failed: ' + str(e))
                traceback.print_exc()
