...
CRITICAL LINK RULE: Whenever you mention a website, government agency, form, tool, app, or online resource ANYWHERE in the guide, you MUST format it as a clickable markdown link using [text](url) format. This applies throughout the entire guide including step-by-step instructions, tips, resource lists, and everywhere else.

CORRECT examples:
- Visit [AnnualCreditReport.com](https://www.annualcreditreport.com) to get your free report
- Download [Form LLC-1](https://www.sos.ca.gov/business-programs/business-entities/forms) from the California Secretary of State
- Use [Credit Karma](https://www.creditkarma.com) to monitor your score for free
- File online at [IRS.gov](https://www.irs.gov/filing/free-file-do-your-federal-taxes-for-free)

WRONG examples (never do this):
- Visit AnnualCreditReport.com to get your free report
- Go to https://www.irs.gov to file
- Download the form from the California Secretary of State website

Every single website, tool, form, or online resource must be a clickable [text](url) link. Never write a URL as plain text. Never mention a website without linking it.''',
        ...
        # Debug: count links in returned markdown
        md = parsed.get('markdown', '')
        link_count = len(_re.findall(r'\[([^\]]+)\]\(([^)]+)\)', md))
        print(f'DEBUG generate_guide: markdown has {link_count} links, length={len(md)}')
