...
def send_guide_email(to_email, customer_name, guide_title, pdf_url, extra_files=None):
    ...
    # Download PDF from Dropbox — follow redirects and verify content
    pdf_response = req.get(pdf_url, allow_redirects=True, timeout=30)
    pdf_response.raise_for_status()

    # Verify we got actual PDF content not an HTML error page
    pdf_data = pdf_response.content
    if len(pdf_data) < 1000 or pdf_data[:4] != b'%PDF':
        # Try with direct download URL format
        direct_url = pdf_url.replace('www.dropbox.com', 'dl.dropboxusercontent.com')
        direct_url = direct_url.replace('?dl=1', '').replace('?dl=0', '')
        pdf_response2 = req.get(direct_url, allow_redirects=True, timeout=30)
        pdf_response2.raise_for_status()
        pdf_data = pdf_response2.content

    if len(pdf_data) < 1000:
        raise Exception(f'PDF download failed — got {len(pdf_data)} bytes from {pdf_url}')
