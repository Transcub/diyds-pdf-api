...
def generate_and_publish():
    ...
    try:
        ...
        # Upload to Dropbox and create Stripe link
        dropbox_url = upload_to_dropbox(pdf_data, filename)
        stripe_url = create_stripe_payment_link(title, price, description)

        tag_list = tags if isinstance(tags, list) else [t.strip() for t in tags.split(',') if t.strip()]

        # Return all data to admin panel — admin panel handles GitHub update
        return jsonify({
            'success': True,
            'title': title,
            ...
            'pdf': dropbox_url,
            'tags': tag_list,
            'message': f'PDF generated and uploaded. Guide ready to publish.'
        })
