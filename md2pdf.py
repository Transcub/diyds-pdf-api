...
def parse_inline(text):
    ...
    # Extract links BEFORE escaping so URLs are preserved
    links = []
    def save_link(m):
        label = m.group(1)
        url = m.group(2)
    ...
    # Restore links with properly escaped URL (& -> &amp; in URL for XML validity)
    for i, (label, url) in enumerate(links):
        xml_url = url.replace('&', '&amp;')
        text = text.replace(
            f'__LINK_{i}__',
            f'<link href="{xml_url}"><font color="#0066CC"><u>{label}</u></font></link>'
        )
    return text
...
def build_story(title, blocks):
    ...
    for block in blocks:
        ...
        if t == 'space':
            ...
            # Check if it's a Phase heading — give it special treatment
            text = block['text']
            is_phase = text.lower().startswith('phase')
            if is_phase:
                story.append(sp(8))
                story.append(Paragraph(
                    f'<font color="#ffffff">&nbsp;&nbsp;{text.upper()}&nbsp;&nbsp;</font>',
                    ParagraphStyle('phase', fontName='Helvetica-Bold', fontSize=11, leading=16,
                                   textColor=WHITE, backColor=colors.HexColor('#3a3a3a'),
                                   spaceBefore=16, spaceAfter=10,
                                   leftIndent=-6, rightIndent=-6,
                                   borderPadding=(7, 10, 7, 10),
                                   borderColor=LIME, borderWidth=0,
                                   borderRadius=0)
                ))
                # Lime underline accent
                story.append(HRFlowable(width='100%', thickness=2,
                                        color=LIME, spaceBefore=0, spaceAfter=8))
            else:
                story.append(Paragraph(parse_inline(text), sH3))
