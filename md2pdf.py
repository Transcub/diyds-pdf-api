...
def header_footer(canvas, doc):
    ...
    canvas.drawString(MARGIN, H - 21, 'DOITYADAMNSELF.COM')
    canvas.setFillColor(GRAY)
    canvas.setFont(FONT, 8)
    canvas.drawRightString(W - MARGIN, H - 21, 'Your DIY Guide to Getting It Done')
    # Footer
    canvas.setFillColor(LIME)
    canvas.rect(0, 0, W, 26, fill=1, stroke=0)
    canvas.setFillColor(BLACK)
    canvas.setFont(FONT, 7.5)
    canvas.drawString(MARGIN, 8, '© DoItYaDamnSelf.com — For personal use only. Not legal advice.')
